...
const ToolbarBtn = ({ onClick, active, title, children }) => (
  <button
    ...
    className={`w-7 h-7 flex items-center justify-center rounded text-xs font-semibold transition-colors ${
      active ? 'bg-violet-100 text-violet-600' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'
    }`}
  >
    {children}
  </button>
...
export function RichTextEditor({ value, onChange, placeholder, rows = 6 }) {
    ...
    editorProps: {
      ...
      handlePaste: (view, event) => {
        ...
        const isTrulyRich = richHtml && (
          richHtml.includes('docs-internal-guid') ||   
          richHtml.includes('mso-') ||                  
          richHtml.includes('urn:schemas-microsoft') || 
          richHtml.includes('xmlns:o=')                 
        );
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-1 focus-within:ring-violet-400/50 focus-within:border-transparent transition-all bg-white">
      <div className="flex items-center gap-0.5 px-2 py-1.5 border-b border-slate-200 bg-slate-50 flex-wrap">
        {groups.map((group, gi) => (
          <div key={gi} className="flex items-center gap-0.5">
            {gi > 0 && <div className="w-px h-4 bg-slate-200 mx-1" />}
            {group.map((btn) => (
        ...
        <div className="ml-auto flex items-center gap-0.5">
          <label title="Inserir imagem" className="w-7 h-7 flex items-center justify-center rounded cursor-pointer text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors">
      ...
      <div className="px-3 py-2.5 bg-slate-50 focus-within:bg-white transition-colors">
