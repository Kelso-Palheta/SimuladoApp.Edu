...
const BimestreCard = ({ numero, turma, alunoId }) => {
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
      <h3 className="text-xs font-semibold text-slate-900 uppercase tracking-wider mb-3">{numero}º Bimestre</h3>
      {atividades.length > 0 || nota?.simulado != null ? (
        <>
          <div className="flex justify-between text-sm mb-1">
            <span className="text-slate-400">Simulado</span>
            <span className="font-mono text-slate-900 tabular-nums">{nota?.simulado != null && nota?.simulado !== '' ? fmt(nota.simulado) : '0,00'}</span>
          </div>
          {atividades.map((atv) => {
            const v = nota?.[atv.id];
            return (
              <div key={atv.id} className="flex justify-between text-sm mb-1">
                ...
                <span className="font-mono text-slate-900 tabular-nums">{v != null && v !== '' ? fmt(v) : '0,00'} / {fmt(atv.max)}</span>
              </div>
            );
          })}
        ...
        <p className="text-sm text-slate-400">Nenhuma atividade neste bimestre.</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-semibold text-slate-900 truncate">{atividade.titulo}</p>
        ...
        <a
          ...
          className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
            !entrega
              ...
              : 'bg-white border border-slate-200 hover:border-violet-300 text-slate-900'
          }`}
        >
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="text-center max-w-sm animate-card-in">
          ...
          <h1 className="text-lg font-bold text-slate-900 mb-2">Sem dados</h1>
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 sm:p-5 mb-8">
          ...
          <div className="border-t border-slate-200 pt-4">
            <div className="flex justify-between items-baseline">
              <span className="text-sm font-semibold text-slate-900">Total Anual</span>
        ...
        {atividades.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3">Atividades</h2>

            <div className="flex gap-1 mb-4 bg-slate-50 border border-slate-200 rounded-xl p-1">
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                    abaAtv === key ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-700'
                  }`}
                >
                  {label}
                  {count > 0 && (
                    <span className={`w-4 h-4 rounded-full text-[10px] flex items-center justify-center ${
                      abaAtv === key ? 'bg-violet-500 text-white' : 'bg-slate-200 text-slate-400'
                    }`}>{count}</span>
                  )}
                </button>
              ))}
