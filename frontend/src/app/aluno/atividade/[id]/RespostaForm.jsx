  ...
  return (
    <div className="mb-6">
      ...
      {questao.imagens?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {questao.imagens.map((img, i) => (
            <img key={i} src={img.url || img.base64} alt={`Imagem ${i + 1}`} className="max-h-48 rounded-lg border border-slate-200 object-contain" />
          ))}
        </div>
      )}
      ...
      <textarea
        ...
        rows={5}
        maxLength={5000}
        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 outline-none focus:bg-white focus:ring-1 focus:ring-violet-400/50 transition-all resize-y"
      />
